...
async function cloudCount(){const c=cloudConfig();if(!c)return null;const res=await fetch(`${c.SUPABASE_URL}/rest/v1/reservations?select=id`,{headers:{apikey:c.SUPABASE_ANON_KEY,Authorization:`Bearer ${c.SUPABASE_ANON_KEY}`,Prefer:"count=exact"}});if(!res.ok)throw new Error("count failed");const range=res.headers.get("content-range");return range?Number(range.split("/")[1]):(await res.json()).length}
async function cloudReserve(data){const c=cloudConfig();if(!c)return null;const res=await fetch(`${c.SUPABASE_URL}/rest/v1/reservations`,{method:"POST",headers:{apikey:c.SUPABASE_ANON_KEY,Authorization:`Bearer ${c.SUPABASE_ANON_KEY}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(data)});if(!res.ok){const body=await res.text();if(res.status===409||body.includes("duplicate"))throw new Error("duplicate");throw new Error("save failed")}return true}
async function cloudSubscribe(email){const c=cloudConfig();if(!c)return null;const res=await fetch(`${c.SUPABASE_URL}/rest/v1/newsletter_subscribers`,{method:"POST",headers:{apikey:c.SUPABASE_ANON_KEY,Authorization:`Bearer ${c.SUPABASE_ANON_KEY}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({email})});if(!res.ok){const body=await res.text();if(res.status===409||body.includes("duplicate"))return "duplicate";throw new Error("subscribe failed")}return "saved"}
